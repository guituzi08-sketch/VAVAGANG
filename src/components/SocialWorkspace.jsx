...
function PersonRow({ person, children }) {
  ...
  const online = person.status === "online" || person.presenceStatus === "online";
  return <article className="person-row"><div className="avatar avatar-fallback">{name[0] ?? "?"}</div><div><strong>{name}</strong><span>{person.username ? `@${person.username} · ` : ""}<i className={online ? "online-label" : ""}>{online ? "Online" : "Offline"}</i></span></div><div className="person-actions">{children}</div></article>;
}
...
function GroupsView({ groupId }) {
  const { groups, createGroup, deleteGroup, createChannel, deleteChannel, inviteUser, invites, respondInvite } = useSocial();
  ...
  if (!groupId) return <GroupsList groups={groups} onCreate={() => setShowCreate(true)} onOpen={(id) => navigate(`/groups/${id}`)} modal={showCreate ? <GroupModal onClose={() => setShowCreate(false)} onCreate={async (data) => { const createdId = await createGroup(data); setShowCreate(false); navigate(`/groups/${createdId}`); }} /> : null} />;
  ...
  const categories = [...new Set((group.channels ?? []).map((channel) => channel.category))]; const role = group.roles?.[firebaseUser?.uid] ?? (group.ownerId === firebaseUser?.uid ? "OWNER" : "MEMBER"); const canManage = role === "OWNER" || role === "ADMIN";
  return <div className="social-content"><SocialHeader eyebrow="Grupo" title={group.name} description={group.description || "Comunidade Vavagang"} action={group.ownerId === firebaseUser?.uid && <button className="danger-button" onClick={async () => { await deleteGroup(group.id); navigate("/groups"); }}><Trash2 size={15} /> Excluir grupo</button>} /><div className="group-layout"><aside className="group-channel-list"><div className="channel-heading"><span>Canais</span><button className="nav-add" onClick={() => setShowChannel(true)} title="Criar canal"><Plus size={15} /></button></div>{categories.map((category) => <div key={category}><span className="nav-caption">{category}</span>{group.channels.filter((channel) => channel.category === category).map((channel) => <button className="group-channel-link" key={channel.id} onClick={() => navigate(channel.type === "VOICE" ? `/voice/${channel.id}` : `/channels/${channel.id}`)}><span>{channel.type === "VOICE" ? "🔊" : "#"}</span>{channel.name}<MoreHorizontal size={14} onClick={async (event) => { event.stopPropagation(); await deleteChannel(group.id, channel.id); }} /></button>)}</div>)}</aside><section className="group-overview"><UsersRound size={26} /><h2>{group.name}</h2><p>{group.description || "Escolha um canal para começar."}</p><span className="privacy-badge">{group.privacy === "private" ? "🔒 Privado" : "🌎 Público"}</span><button className="secondary-button" onClick={() => setShowMembers(true)}>Membros ({group.memberIds?.length ?? 1})</button></section></div>{showChannel && <ChannelModal onClose={() => setShowChannel(false)} onCreate={async (data) => { await createChannel(group.id, data); setShowChannel(false); }} />}{showMembers && <GroupMembersPanel group={group} role={role} canManage={canManage} profile={profile} firebaseUser={firebaseUser} inviteUser={inviteUser} invites={invites.filter((invite) => invite.groupId === group.id)} respondInvite={respondInvite} onClose={() => setShowMembers(false)} />}</div>;
}

function GroupMembersPanel({ group, role, canManage, profile, firebaseUser, inviteUser, invites, respondInvite, onClose }) {
  const [query, setQuery] = useState(""); const [results, setResults] = useState([]); const [message, setMessage] = useState(""); const [memberProfiles, setMemberProfiles] = useState({});
  useEffect(() => { let active = true; Promise.all((group.memberIds ?? [group.ownerId]).map((uid) => getUserProfile(uid))).then((profiles) => { if (active) setMemberProfiles(Object.fromEntries(profiles.map((item) => [item.uid, item]))); }); return () => { active = false; }; }, [group.memberIds, group.ownerId]);
  useEffect(() => { if (!query.trim()) return setResults([]); const timer = window.setTimeout(async () => setResults(await searchUsers(query)), 300); return () => window.clearTimeout(timer); }, [query]);
  const permissions = { OWNER: "Gerenciar membros, cargos, convites, canais e grupo", ADMIN: "Gerenciar membros, convites e canais", MODERATOR: "Moderar mensagens e canais", MEMBER: "Participar, conversar, voz e tela" };
  async function invite(user) { try { await inviteUser(group.id, user); setMessage("Convite enviado."); } catch (error) { setMessage(error.message); } }
  return <div className="composer-backdrop" onClick={onClose}><section className="composer-modal" onClick={(event) => event.stopPropagation()}><button className="composer-close" onClick={onClose}><X size={17} /></button><h2>Membros</h2>{(group.memberIds ?? [group.ownerId]).map((uid) => { const memberRole = group.roles?.[uid] ?? (uid === group.ownerId ? "OWNER" : "MEMBER"); const member = memberProfiles[uid] ?? {}; return <article className="person-row" key={uid}><div className="avatar avatar-fallback">{(member.nickname || member.displayName || uid)[0]}</div><div><strong>{member.nickname || member.displayName || uid}</strong><span>{member.presenceStatus ?? "offline"} · {memberRole} · {permissions[memberRole]}</span></div>{canManage && uid !== group.ownerId && <select className="settings-select" defaultValue={memberRole} onChange={async (event) => { await changeMemberRole(group.id, uid, event.target.value); }}><option>MEMBER</option><option>MODERATOR</option><option>ADMIN</option></select>}</article>; })}<h3>Convidar pessoas</h3><input className="modal-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar nickname" />{results.filter((user) => !group.memberIds?.includes(user.uid)).map((user) => <button className="small-action" key={user.uid} onClick={() => invite(user)}>{user.nickname || user.displayName} · Enviar convite</button>)}{invites.map((invite) => <p key={invite.id}>{invite.recipientId} · aguardando resposta</p>)}{message && <p className="settings-feedback">{message}</p>}<p className="muted">Seu cargo: {role}. {permissions[role]}</p></section></div>;
}
