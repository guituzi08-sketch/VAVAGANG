...
function VoiceChannelContent({ roomId }) {
  ...
  return <div className="voice-channel-page">
    ...
    <div className="voice-channel-grid">
      ...
      <section className="sound-effects-panel"><div className="voice-section-title"><span><Music2 size={16} /> Efeitos sonoros</span><span className="chat-live">compartilhado</span></div><div className="sound-effect-list">{effects.length === 0 && <p className="voice-empty">Adicione um efeito para a sala.</p>}{effects.map((effect) => <div className="sound-effect-row" key={effect.id}><button className="sound-effect-button" onClick={() => trigger(effect)} title={`Reproduzir ${effect.name}`}><Music2 size={15} /><span>{effect.name}</span><small>tocar</small></button>{effect.createdBy === firebaseUser?.uid && <button className="icon-button sound-effect-delete" onClick={async () => { if (!window.confirm(`Excluir o efeito \"${effect.name}\"?`)) return; try { await remove(effect); } catch {} }} title="Excluir efeito"><Trash2 size={15} /></button>}</div>)}</div><form className="sound-effect-form" onSubmit={submitSoundEffect}><input value={soundName} onChange={(event) => setSoundName(event.target.value)} placeholder="Nome opcional" maxLength={60} /><label className="sound-upload-button" title="Selecionar MP3 ou WAV"><Upload size={15} /><span>Adicionar áudio</span><input name="soundFile" type="file" accept="audio/mpeg,audio/wav,audio/x-wav" /></label><button className="secondary-button sound-upload-submit" type="submit">Enviar</button></form>{soundError && <p className="error-message">{soundError}</p>}{audioBlocked && <button className="secondary-button sound-unlock" onClick={() => setAudioBlocked(false)}>Ativar efeitos</button>}</section>
    </div>
