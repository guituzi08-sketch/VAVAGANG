...
export default function WorkspacePage({ section }) {
  ...
  return <div className="workspace-page"><div className="workspace-page-header"><div><p className="eyebrow">{content.eyebrow}</p><h1>{content.title}{identifier && <span className="page-id"> / {identifier}</span>}</h1><p className="muted">{content.description}</p></div><NavLink className="search-field" to="/search" title="Pesquisar"><Search size={16} /> Pesquisar</NavLink></div><div className="workspace-empty"><div className="empty-icon"><Icon size={24} /></div><h2>{section === "settings" ? "Preferências" : "Nenhum dado ainda"}</h2><p>{section === "friends" ? "Adicione pessoas reais para começar sua rede." : section === "messages" ? "Quando você iniciar uma conversa, ela aparecerá nesta área." : "Os dados aparecerão aqui assim que forem criados."}</p><div className="empty-line"><Bell size={15} /> Nenhum dado foi criado ainda</div></div></div>;
  return <div className="workspace-page"><div className="workspace-page-header"><div><p className="eyebrow">{content.eyebrow}</p><h1>{content.title}{identifier && <span className="page-id"> / {identifier}</span>}</h1><p className="muted">{content.description}</p></div><NavLink className="search-field" to="/search" title="Pesquisar"><Search size={16} /> Pesquisar</NavLink></div><div className="workspace-empty"><div className="empty-icon"><Icon size={24} /></div><h2>{section === "settings" ? "Preferências" : "Nenhum dado ainda"}</h2><p>{section === "friends" ? "Adicione pessoas reais para começar sua rede." : section === "messages" ? "Quando você iniciar uma conversa, ela aparecerá nesta área." : "Os dados aparecerão aqui assim que forem criados."}</p><div className="empty-line"><Bell size={15} /> Nenhum dado foi criado ainda</div></div></div>;
}